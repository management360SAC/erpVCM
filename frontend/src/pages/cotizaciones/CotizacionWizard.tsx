// src/pages/cotizaciones/CotizacionWizard.tsx
...
import {
  ...
  DialogContent, DialogActions, Autocomplete, MenuItem,
} from "@mui/material";
...
import { sendQuoteEmail, sendQuickQuoteEmail, type Currency } from "../../apis/quotes";
...
const CURRENCY_LABEL: Record<Currency, string> = { PEN: "Soles (PEN)", USD: "Dólares (USD)" };
...
async function makeA4PdfFromNode(node: HTMLElement): Promise<Blob> {
  ...
  const sliceHeightPx = Math.floor(usableHmm / pxToMm) - 20;
...
/* =================== Componente =================== */
export interface CotizacionWizardProps {
  /** "full": página completa /cotizaciones/nueva, guarda en el historial.
   *  "quick": modal desde el Embudo de Ventas, NO se guarda en el historial. */
  mode?: "full" | "quick";
  /** Deal del embudo al que se vincula (solo mode="quick"). Al enviar, mueve el deal a PROPUESTA. */
  relatedDealId?: number | null;
  /** Precarga de cliente cuando se abre desde una oportunidad existente. */
  initialClientId?: number | null;
  initialLegalName?: string;
  /** Se llama al aceptar el diálogo de éxito (en vez de navegar al historial). */
  onDone?: () => void;
}
...
export default function CotizacionWizard({
  ...
  // Moneda (solo etiqueta/símbolo, sin conversión de montos)
  const [currency, setCurrency] = useState<Currency>("PEN");
  const fmt = (n: number) =>
    `${CURRENCY_SYMBOL[currency]} ${n.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
  ...
  // Auth guard (solo aplica a la página completa; el modal ya vive detrás de una ruta privada)
  useEffect(() => {
    if (isQuick) return;
  ...
  // Precargar el cliente vinculado a la oportunidad (mode="quick")
  useEffect(() => {
    if (!initialClientId || clientsList.length === 0 || client) return;
    const match = clientsList.find((c) => c.id === initialClientId);
    if (match) {
      setClient(match);
      setForm((f) => ({
        ...f,
  ...
  }, [initialClientId, clientsList]); // eslint-disable-line
  ...
  const handleSendQuote = async () => {
    ...
    try {
      ...
      const filename = `${isQuick ? "COT-RAPIDA" : "COT"}-${client?.id ?? "preview"}.pdf`;
      ...
      const payload = {
        ...
        meta: {
          ...
          ...(isQuick && relatedDealId ? { relatedDealId } : {}),
        },
      ...
      if (isQuick) {
        await sendQuickQuoteEmail(payload, blob, filename);
      } else {
      ...
      setResultMsg(
        isQuick
          ? "Cotización rápida enviada correctamente al correo. No se guardó en el historial de cotizaciones."
          : "Cotización enviada correctamente al correo."
      );
      setResultOpen(true);
    } catch (e: any) {
      console.error(e);
  ...
  const handleDownloadPDF = async () => {
    ...
    a.href = url;
    a.download = `${isQuick ? "COT-RAPIDA" : "COT"}-${client?.id ?? "preview"}.pdf`;
    a.click();
        ...
        <Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems={{ sm: "center" }} justifyContent="space-between" sx={{ mb: 2 }}>
          ...
          >
            {(Object.keys(CURRENCY_LABEL) as Currency[]).map((c) => (
              <MenuItem key={c} value={c}>{CURRENCY_LABEL[c]}</MenuItem>
            ))}
          </TextField>
        </Stack>
              ...
              <Table>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell align="right" width={220}>Costo ({CURRENCY_SYMBOL[currency]})</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  ...
                  <TableRow>
                    ...
                    <TableCell align="right">{fmt(igv)}</TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell colSpan={2} align="right" sx={{ fontWeight: 800 }}>Total:</TableCell>
                    <TableCell align="right" sx={{ fontWeight: 800 }}>{fmt(total)}</TableCell>
                  </TableRow>
                </TableBody>
              </Table>
            </TableContainer>
        ...
        <Box>
              ...
              <Table size="small" sx={{ mb: 2, border: "1px solid #e0e0e0" }}>
                <TableHead>
                  <TableRow sx={{ backgroundColor: "#f5f5f5" }}>
                    ...
                    <TableCell align="right" sx={{ fontWeight: 800, width: 150, fontSize: "0.9rem" }}>Costo ({CURRENCY_SYMBOL[currency]})</TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                    ...
                    selectedServicesData.map((s) => {
                      ...
                      return (
                        <TableRow key={s.id}>
                          ...
                          <TableCell align="right" sx={{ fontSize: "0.88rem" }}>{fmt(cost)}</TableCell>
                        </TableRow>
                      );
                    })
                  )}
                  <TableRow sx={{ backgroundColor: "#fafafa" }}>
                    ...
                    <TableCell align="right" sx={{ fontSize: "0.9rem" }}>{fmt(subTotal)}</TableCell>
                  </TableRow>
                  <TableRow sx={{ backgroundColor: "#fafafa" }}>
                    <TableCell align="right" sx={{ fontWeight: 700, fontSize: "0.9rem" }}>IGV (18%):</TableCell>
                    <TableCell align="right" sx={{ fontSize: "0.9rem" }}>{fmt(igv)}</TableCell>
                  </TableRow>
                  <TableRow sx={{ backgroundColor: "#e3f2fd" }}>
                    <TableCell align="right" sx={{ fontWeight: 800, fontSize: "0.95rem" }}>Total:</TableCell>
                    <TableCell align="right" sx={{ fontWeight: 800, fontSize: "0.95rem" }}>{fmt(total)}</TableCell>
                  </TableRow>
                </TableBody>
              </Table>
          ...
          <Stack direction="row" spacing={2} justifyContent="center" sx={{ mt: 3 }}>
            ...
            <Button variant="contained" startIcon={<SendIcon />} onClick={handleSendQuote}>
              {isQuick ? "Enviar Cotización Rápida" : "Enviar Cotización"}
            </Button>
          </Stack>
        </Box>
      )}
